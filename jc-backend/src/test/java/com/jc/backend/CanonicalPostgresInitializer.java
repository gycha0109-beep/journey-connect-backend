...
public final class CanonicalPostgresInitializer
    ...
    private static final List<String> SCRIPTS = List.of(
            "01_initial_schema.sql", "02_seed.sql", "03_smoke_test.sql",
            "04_admin_support.sql", "05_security_roles.sql", "06_security_smoke_test.sql",
            "07_recommendation_storage.sql", "08_recommendation_security_roles.sql",
            "09_recommendation_smoke_test.sql", "10_backend_runtime.sql",
            "11_backend_runtime_security_roles.sql", "12_backend_runtime_smoke_test.sql",
            ...
            "55_search_exposure_persistence.sql", "55a_search_exposure_digest_privilege.sql",
            "56_search_exposure_persistence_smoke_test.sql",
    ...
    private static final String EXTERNAL_PASSWORD = setting("jc.test.db.password", "JC_TEST_DB_PASSWORD", "postgres");
    private static final boolean EXTERNAL_RESET = Boolean.parseBoolean(setting("jc.test.db.reset", "JC_TEST_DB_RESET", "false"));
    ...
    private static void applyProperties(ConfigurableApplicationContext context, String jdbcUrl, String username, String password) {
    ...
    private static PostgreSQLContainer<?> createContainer() {
        String image = setting("jc.test.postgres.image", "JC_TEST_POSTGRES_IMAGE", "postgres:15-alpine");
        PostgreSQLContainer<?> container = new PostgreSQLContainer<>(image)
                .withDatabaseName("journey_connect").withUsername("postgres").withPassword("postgres");
        for (String script : SCRIPTS) {
            container.withCopyFileToContainer(MountableFile.forClasspathResource("db/canonical/" + script), "/sql/" + script);
        }
        return container;
    }
    ...
    private static synchronized void ensureContainerStarted() {
        ...
        for (String script : SCRIPTS) {
            try {
                Container.ExecResult result = POSTGRES.execInContainer("psql", "-v", "ON_ERROR_STOP=1", "-U", POSTGRES.getUsername(), "-d", POSTGRES.getDatabaseName(), "-f", "/sql/" + script);
                if (result.getExitCode() != 0) {
                    throw new IllegalStateException("Canonical PostgreSQL bootstrap failed at " + script + "\nstdout:\n" + result.getStdout() + "\nstderr:\n" + result.getStderr());
                }
            } catch (Exception exception) {
                POSTGRES.stop();
                throw new IllegalStateException("Canonical PostgreSQL bootstrap failed at " + script, exception);
            }
        }
    }
    ...
    private static synchronized void ensureExternalReady() {
        ...
        if (EXTERNAL_RESET) CanonicalSqlBootstrapper.resetAndApply(EXTERNAL_URL, EXTERNAL_USERNAME, EXTERNAL_PASSWORD, SCRIPTS);
        externalReady = true;
    }
