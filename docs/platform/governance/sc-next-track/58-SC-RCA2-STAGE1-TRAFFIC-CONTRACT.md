# Stage 1 Traffic Contract

## Purpose

Define the candidate non-production Stage 1 ceiling without enabling it.
...
`TARGET_TRAFFIC_PERCENT=1` is a ceiling candidate, not an effective setting. `CURRENT_TRAFFIC_PERCENT=0` remains authoritative until every blocker is closed and a separate manual decision is recorded.
...
Actual traffic, endpoint, credential, allowlist, observation, production route/identity/traffic, candidate serving and authority transfer are `NOT_EXECUTED`.
