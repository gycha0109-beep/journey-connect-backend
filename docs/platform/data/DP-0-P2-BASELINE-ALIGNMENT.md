...
| 원 P2 검증 기준 | `journey-connect-db-v2.7`, canonical SQL `01..26` |
| 작성 기준일 | 2026-07-19 |
| SC 보정일 | 2026-07-21 |
...
위 `01..26` PASS는 원 P2 검증 증거다. SQL 27/28은 current canonical sequence에 포함되지만 이 역사적 결과를 소급 확장하지 않는다.
...
6. 신규 profile source/schema/consumer version 명시
...
7. metric definition version과 dataset schema/consumer version 결속
8. canonical bytes/content hash reproducibility
9. P2 23개 core 및 backend contract gate PASS
...
- Data dataset과 P2 dataset을 연결할 때 승인된 restricted identity mapping port를 사용한다.
- mapping 실패를 anonymous 또는 다른 subject로 fallback하지 않는다.
- opaque subject로 P2 평가 입력을 전환하면 새 dataset/schema/consumer version과 migration/replay 계획이 필요하다.
...
- ownership 이전 또는 port 분리는 별도 High-risk handoff와 전체 회귀가 필요한 SC 사안
- Data Platform은 현재 object에 read-only 승인 없이 접근하지 않고 절대 write하지 않는다.

## 10. DB 기준선 영향

- 현재 canonical DB 기준은 `journey-connect-db-v2.7/01..28`
- Data Platform은 SQL `25..28`을 수정하지 않는다.
- SQL 27의 `search_document_projection_v1`은 Search-owned rebuildable derived projection이며 Data authority가 아니다.
- SQL 27의 `search_document_operational_eligibility_v1`은 Operations semantic authority이며 missing row fail-closed 의미를 유지한다.
- SQL 28은 SQL 27 smoke test다.
- DP-2 신규 DB 번호는 SC가 `28` 이후 기준으로 배정해야 한다.
- 신규 Data schema가 같은 DB에 추가되어도 P1/P2/Search/Operations table write owner와 role/grant를 침범하지 않는다.
...
| DP-2 | canonical DB `v2.7/28` 이후 번호 배정 필요 |
...
| SQL 변경 없음 | PASS |
| 원 P2 테스트 재실행 | 미실행 — 문서 단계 |
| SC reconciliation static/protected diff | machine-readable evidence에서 별도 검증 |
...
- 신규 Data fingerprint exact contract는 `SC DECISION REQUIRED`; DP-1은 fingerprint 구현 전 중단한다.
