...
| 검증 | 결과 | 근거 |
|---|---|---|
| 필수 문서 존재 | PASS | 산출물 10개 생성 |
| 문서 링크 존재 | PASS | 상대 링크 자동 검사 |
| 계약 ID 중복 없음 | PASS | 계약 registry 자동 검사 |
| event type 중복 없음 | PASS | taxonomy registry 자동 검사 |
| wire value lowercase snake_case | PASS | 정규식 검사 |
| 식별자/시간/버전 규칙 일치 | PASS | 문서 교차 검토 |
| P0 source 파일 변경 없음 | PASS | 신규 문서는 기준 프로젝트 외부에 생성 |
| P0 DB SQL 변경 없음 | PASS | `.sql` 생성·수정 없음 |
| P0 event 의미 변경 없음 | PASS | adapter-only 전략 |
| 다른 트랙 direct write 없음 | PASS | architecture/adapter contract에 금지 고정 |
| P2 baseline 검증 증거 확인 | PASS | Batch18 reports, test summary, baseline integrity 확인 |
| P1/P2 runtime source path 확인 | PASS | P1 profile source와 P2 observation/evaluation source 검토 |
| P2 실행 테스트 재실행 | 미실행 | 코드·SQL 비변경 문서 보정 단계; 제공된 Batch18 검증 증거 확인 |
| PostgreSQL runtime 재실행 | 미실행 | DB 변경 없음 |
...
DP-1은 production DB 변경 없이 Java domain type, validator, canonicalizer, fixture, contract test만 구현한다. DB 변경은 System Coordination이 canonical DB version과 SQL sequence를 배정한 후 DP-2에서 시작한다. 현재 canonical 기준은 `journey-connect-db-v2.7/01..26`이며 신규 번호는 그 이후로 배정받아야 한다.
...
- 현재 P1/P2 direct source 경로를 Data projection으로 전환하려면 신규 source/dataset version과 전체 회귀가 필요
- 현재 P2 물리적 write path와 Reliability 논리 소유권의 정합화는 System Coordination 결정 필요
