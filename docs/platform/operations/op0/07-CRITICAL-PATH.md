...
```text
CANDIDATE_SOURCE_DECISION -> ENDPOINT -> CREDENTIAL -> NETWORK_ALLOWLIST -> TEST_ACCOUNT_ALLOWLIST -> COHORT_SELECTION -> CANDIDATE_ADAPTER -> METRICS -> DASHBOARD_ALERT -> ROLLBACK_DRILL -> ROLE_APPROVALS -> MANUAL_ENABLEMENT
```

The path is blocked today at candidate-source decision, external endpoint/repository identification, credential/allowlist implementation, metric completion, dashboard/alerts, rollback drills and six approvals. Manual enablement is never implied by completing an earlier node.
