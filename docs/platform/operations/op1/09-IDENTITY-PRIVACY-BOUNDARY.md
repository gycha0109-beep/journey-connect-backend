...
Raw user IDs are rejected as cohort input. Existing `Rca2IdentityPolicy` blocks `user:<id>` and production subjects. Allowlist and cohort layers receive only hashed audit references. Metrics carry bounded result classes, never subject, token or endpoint URL labels.
