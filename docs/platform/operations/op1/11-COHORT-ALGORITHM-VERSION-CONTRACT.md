# Cohort Algorithm and Version Contract

| Field | Value |
|---|---|
| Official phase | `OP-1 RCA-2 Stage 1 Environment and Access Preparation` |
| Work-start / authoritative main | `0d9cdc7bf9821d5bb8811bfdd08286f3e4804e8d` |
| OP-0 exact final head | `e29a056d63c8c953851e4261bde9f71f3cd19441` |
| OP-0 merge commit | `0d9cdc7bf9821d5bb8811bfdd08286f3e4804e8d` |
| Artifact version | `op1-rca2-stage1-environment-access-v1` |
| Updated at | `2026-07-27T11:00:00Z` |

```text
COHORT_SELECTION=STABLE_HASH_PERCENTAGE
ALGORITHM_VERSION=rca2-stable-hash-percentage-v1
BUCKET_COUNT=10000
COHORT_KEY=HASHED_NONPRODUCTION_TEST_SUBJECT_REF
RAW_IDENTITY_KEY=FORBIDDEN
TOKEN_DERIVED_RANDOM_REQUEST_HASH=FORBIDDEN
REQUEST_RANDOMNESS=FORBIDDEN
MAX_PERCENT_CEILING=1
CURRENT_PERCENT=0
DEFAULT_PERCENT=0
```

Salt version/material are blank in runtime configuration, so actual selection remains blocked even though the implementation and deterministic tests are ready.
