# RCA-1 P2 Shadow Reconciliation Result
...
Executed 39 P2 cases. Baseline lane result is `RECONCILED_WITH_MIGRATION_GAPS`; baseline unexpected mismatch count is zero. Exact P2 exposure authority, assignment/version/variant, hashed subject-session-run-exposure binding, 604800-second window, allowed engagement, and bound fallback are enforced. Output includes `P2_SHADOW_RECONCILIATION_ONLY`, `CURRENT_P2_AUTHORITY_UNCHANGED`, and `NO_AUTHORITY_TRANSFER`.
