# RCA-1 Evidence Schema and Redaction
...
Evidence fields are fixed to hashedCaseId, lane, contractId/version, dimension, classification, normalized expected/actual, redacted checkpoint, lineage fingerprint, evidence timestamp, verifier version, and tested SHA. JSON field order and TSV column order are fixed; duplicate case/dimension records are rejected.
