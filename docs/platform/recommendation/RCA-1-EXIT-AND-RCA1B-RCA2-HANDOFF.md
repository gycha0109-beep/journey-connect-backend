# RCA-1 Exit and RCA-1B/RCA-2 Handoff
...
RCA-1 exits only with separate P1/P2 results, classified differences, enforced synthetic identity, unchanged authority, no production traffic, and no authority transfer. RCA-1B must define read-only non-production DB role, timeout, row limit, reproducible data, checkpoint/freshness behavior, and Operations approval. RCA-2 must separately authorize feature flags, dark-read timeout/fallback, observability, deployment, credentials, traffic controls, rollback, and production identity governance.
