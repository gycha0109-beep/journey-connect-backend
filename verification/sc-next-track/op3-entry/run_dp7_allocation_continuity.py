#!/usr/bin/env python3
"""Run authoritative DP-7 verification across later successor work."""
...
source = source.replace(
    changed_anchor,
    '''    current_base = subprocess.run(["git", "merge-base", "origin/main", "HEAD"], cwd=ROOT,
                                  check=True, text=True, capture_output=True).stdout.strip()
    current_changed = subprocess.run(["git", "diff", "--name-only", f"{current_base}...HEAD"], cwd=ROOT,
                             check=True, text=True, capture_output=True).stdout.splitlines()
