...
EV = ROOT / "verification/sc-next-track/rca1b-entry"
...
def main() -> int:
    ...
    try:
        ...
        def baseline() -> str:
            values = {row["key"]: row["value"] for row in rows(EV / "SC_RCA1B_BASELINE.tsv")}
            need(values.get("work_start_sha") == START, "work-start mismatch")
            need(values.get("rca1_exact_final_head") == RCA1_FINAL, "RCA-1 final-head mismatch")
            sh(["git", "merge-base", "--is-ancestor", START, head])
            return "exact work-start and branch ancestry verified"
        record("exact_authoritative_work_start", baseline, f"git merge-base --is-ancestor {START} {head}")

        def merge_tree() -> str:
            need("Merge pull request #25" in git("show", "-s", "--format=%B", START), "PR #25 merge absent")
            sh(["git", "merge-base", "--is-ancestor", RCA1_FINAL, START])
            sh(["git", "diff", "--quiet", RCA1_FINAL, START])
            return "PR #25 merge and RCA-1 tree equivalence verified"
        record("rca1_merge_and_tree_equivalence", merge_tree, f"git diff --quiet {RCA1_FINAL} {START}")

        def rca1_assets() -> str:
            source = (ROOT / "jc-backend/src/main/java/com/jc/backend/recommendation/dataadoption/reconciliation/Rca1Contracts.java").read_text(encoding="utf-8")
            need(CONTRACTS <= set(re.findall(r'"([a-z0-9-]+-v1)"', source)), "RCA-1 contract missing")
            ...
            need(len(p1) == 23 and len({row["scenario"] for row in p1}) == 23, "P1 fixture inventory mismatch")
            need(len(p2) == 39 and len({row["scenario"] for row in p2}) == 39, "P2 fixture inventory mismatch")
            for marker in ("RECONCILED_WITH_EXPECTED_GAPS", "RECONCILED_WITH_MIGRATION_GAPS", 'IDENTITY_MODE = "SYNTHETIC_ONLY"'):
                need(marker in source, f"RCA-1 marker missing: {marker}")
            return "3 contracts, 23 P1 fixtures, 39 P2 fixtures and lane markers verified"
        record("rca1_contract_fixture_and_lane_baseline", rca1_assets)

        def documents() -> str:
            inventory = rows(EV / "SC_RCA1B_DOCUMENTS.tsv")
            need(len(inventory) == 16 and {Path(row["path"]).name for row in inventory} == set(DOCS), "document inventory mismatch")
            for name in DOCS:
            ...
            return "16 complete SC-4 documents and implementation handoff"
        record("required_documents_and_handoff", documents)

        def decisions() -> str:
            entries = rows(EV / "SC_RCA1B_ENTRY_DECISIONS.tsv")
            values = {row["decision"]: row["value"] for row in entries}
            expected = {
            ...
            for key, value in expected.items():
                need(values.get(key) == value, f"decision mismatch: {key}")
            need(sum(row["decision"] == "environment" for row in entries) == 1, "environment decision not singular")
            return "single environment, version, dataset and identity decisions"
        record("decision_uniqueness", decisions)

        def execution() -> str:
            values = {row["key"]: row["value"] for row in rows(EV / "SC_RCA1B_EXECUTION_CONTRACT.tsv")}
            need(values.get("transaction_isolation") == "REPEATABLE_READ", "isolation mismatch")
            need(values.get("transaction_read_only") == "REQUIRED", "read-only not required")
            for key in ("statement_timeout_ms", "lock_timeout_ms", "idle_in_transaction_timeout_ms",
                ...
                need(int(values.get(key, "0")) > 0, f"finite limit missing: {key}")
            need(values.get("parallel_query") == "DISABLED" and values.get("retry_policy") == "NONE", "parallel/retry mismatch")
            need(values.get("max_checkpoint_lag") == "0" and values.get("lineage_fingerprint") == "REQUIRED", "checkpoint/lineage mismatch")
            return "finite read-only, resource, checkpoint and lineage boundaries"
        record("read_only_resource_checkpoint_contract", execution)

        def query_role_dimensions() -> str:
            need({row["query_id"] for row in rows(EV / "SC_RCA1B_QUERY_ALLOWLIST.tsv")} == QUERIES, "query allowlist mismatch")
            role = {row["property"]: row["value"] for row in rows(EV / "SC_RCA1B_ROLE_GRANT.tsv")}
            ...
            need(role.get("write_grant") == "FORBIDDEN" and role.get("owner_role_use") == "FORBIDDEN", "write/owner policy mismatch")
            dims = rows(EV / "SC_RCA1B_DB_DIMENSIONS.tsv")
            need({row["dimension"] for row in dims if row["lane"] == "P1"} == P1_DIMS, "P1 DB dimensions mismatch")
            need({row["dimension"] for row in dims if row["lane"] == "P2"} == P2_DIMS, "P2 DB dimensions mismatch")
            return "query allowlist, least-privilege role and lane-specific dimensions verified"
        record("query_role_and_lane_boundaries", query_role_dimensions)
        ...
        def governance() -> str:
            ...
            for marker in ("RCA1B_ENTRY_AUTHORIZED", "CI_EPHEMERAL_POSTGRESQL", "POSTGRESQL_VERSION_MATRIX=15,18",
            ...
            need(not re.search(r"\bRP\s*(?:=|:|means)\s*Recommendation(?:\s+Platform)?\b", combined, re.IGNORECASE), "RP naming conflict")
            return "governance, registry, authority and RCA/RP naming aligned"
        record("governance_registry_authority_alignment", governance)
        ...
        def protected() -> str:
            ...
            return "SQL, sources and production defaults protected"
        record("protected_sql_source_and_production_state", protected)
        ...
        def diff_boundary() -> str:
            ...
            allowed = (
                ...
                "verification/sc-next-track/rca1-entry/run_sc_rca1_entry_verification.py",
            ...
            protected_paths = [path for path in changed if path.startswith((
            ...
            need(not protected_paths, f"historical/protected diff: {protected_paths}")
        ...
        for row in rows(EV / "SC_RCA1B_VERIFICATION_PLAN.tsv"):
            if row["sc4_status"] in {"NOT_EXECUTED", "NOT_APPLICABLE"}:
                checks.append({"check": row["check"], "status": row["sc4_status"],
                               "command": row["sc4_status"], "detail": "SC-4 governance scope"})
