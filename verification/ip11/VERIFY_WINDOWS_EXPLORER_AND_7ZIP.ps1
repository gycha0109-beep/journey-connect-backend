param(
  ...
  [string]$WorkRoot = (Join-Path $env:TEMP "JC-IP-11-5-Verify")
)
...
# Windows Explorer/Shell extraction engine.
...
if ($rootNames.Count -ne 1 -or $rootNames[0] -ne 'JC-IP-11-5-Final') { throw "Unexpected top-level folder: $($rootNames -join ',')" }
