...
19. `19_recommendation_replay_audit.sql`
20. `20_recommendation_replay_audit_smoke_test.sql`
21. `21_recommendation_behavior_runtime.sql`
22. `22_recommendation_behavior_runtime_smoke_test.sql`

`03`, `06`, `09`, `12`, `14`, `16`, `18`, `20`, `22`는 마지막에 `ROLLBACK`되므로 테스트 데이터가 남지 않습니다. 역할을 생성·검증하는 스크립트는 PostgreSQL 슈퍼유저 또는 **해당 객체 소유권과 `CREATEROLE` 권한을 모두 가진 계정**으로 실행합니다. 보안 smoke test는 실제 `SET ROLE` 검증이 가능해야 합니다.

> 빈 DB에는 v2.5의 `01~22`를 순서대로 적용합니다. 이미 v2.4를 적용한 DB에는 `21~22`만 실행합니다. v2.3 적용 DB는 `19~22`, v2.2 적용 DB는 `17~22`, v2.1 적용 DB는 `15~22`, v2.0 적용 DB는 `13~22`, v1.9 적용 DB는 `10~22`, v1.8 적용 DB는 `07~22`를 순서대로 적용합니다.
...
- 저장 snapshot 기반 exact replay audit과 CANARY readiness 증거 저장
- run-bound view·click·share·hide·report 행동 저장과 좋아요·북마크 상태 변경의 원자적 이벤트 기록
...
- run-bound behavior는 run의 사용자·세션뿐 아니라 실제 ranked candidate 포함 여부까지 검증합니다.
- 좋아요·취소·저장·저장 취소는 `jc_app`이 보안 함수 하나로 상태와 behavior를 같은 트랜잭션에서 처리하며, 상태 변화가 없으면 새 이벤트를 만들지 않습니다.
- event ID와 idempotency key는 advisory transaction lock으로 직렬화해 동시 충돌 시 한 상태 변경과 한 이벤트만 허용합니다.
...
- 일반 추천 behavior persistence adapter는 `jc_recommendation` 연결을 사용하고 `jc_app`으로 추천 이력을 직접 기록하지 않음
- 좋아요·북마크는 `jc_app`의 `apply_recommendation_post_interaction` 보안 함수로만 상태와 이벤트를 원자 처리
- idempotency key 충돌 시 기존 payload fingerprint를 비교해 동일 재시도와 409 충돌을 구분

## 검증 기준

v1.8 기준선인 `01~06`은 PostgreSQL 15.18과 18.4에서 전체 순차 실행, `05` 재실행, 실제 `SET ROLE` 보안 테스트를 통과했습니다. v1.9의 `07~09`, v2.0의 `10~12`, v2.1의 `13~14`, v2.2의 `15~16`, v2.3의 `17~18`, v2.4의 `19~20`, v2.5의 `21~22`는 PostgreSQL 통합 테스트와 Java 정적 계약 게이트에 포함됩니다. v2.5 최종 검증은 PostgreSQL 15.18에서 `01~22` 전체 순차 실행을 통과했으며, 실제 적용 환경에서도 `22`까지 통과해야 완료로 판정합니다. 데이터베이스 소유자이면서 `CREATEROLE`인 비슈퍼유저로 `01~05`를 적용하는 경로도 두 버전에서 통과했습니다. 세부 내역은 `REVIEW.md`를 확인합니다.
