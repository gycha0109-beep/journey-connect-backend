...
`03`, `06`, `09`는 마지막에 `ROLLBACK`되므로 테스트 데이터가 남지 않습니다. `05`, `08`은 PostgreSQL 슈퍼유저 또는 **해당 객체 소유권과 `CREATEROLE` 권한을 모두 가진 계정**으로 실행합니다. `06`, `09`는 실제 `SET ROLE` 검증을 위해 개발 DB의 PostgreSQL 슈퍼유저로 실행합니다.

> 빈 DB에는 v1.9의 `01~09`를 순서대로 적용합니다. 이미 v1.8을 적용한 DB에는 `01~06`을 재실행하지 말고 `07~09`만 실행합니다. v1.7 이하는 v1.8 차이 마이그레이션을 먼저 완료해야 합니다.
...
실제 로그인 계정은 환경에서 따로 만들고 해당 그룹 역할만 부여합니다. 비밀번호는 SQL 파일이나 Git에 저장하지 않습니다.

```sql
CREATE ROLE journey_app_login LOGIN PASSWORD '환경변수로 관리';
GRANT jc_app TO journey_app_login;

CREATE ROLE journey_auth_login LOGIN PASSWORD '환경변수로 관리';
GRANT jc_auth TO journey_auth_login;

CREATE ROLE journey_admin_login LOGIN PASSWORD '환경변수로 관리';
GRANT jc_admin TO journey_admin_login;

CREATE ROLE journey_recommendation_login LOGIN PASSWORD '환경변수로 관리';
GRANT jc_recommendation TO journey_recommendation_login;
```
...
v1.8 기준선인 `01~06`은 PostgreSQL 15.18과 18.4에서 전체 순차 실행, `05` 재실행, 실제 `SET ROLE` 보안 테스트를 통과했습니다. v1.9의 `07~09`는 PostgreSQL 15·18 service 기반 CI와 정적 계약 게이트를 추가했으며, 실제 적용 환경에서도 `09`까지 통과해야 완료로 판정합니다. 데이터베이스 소유자이면서 `CREATEROLE`인 비슈퍼유저로 `01~05`를 적용하는 경로도 두 버전에서 통과했습니다. 세부 내역은 `REVIEW.md`를 확인합니다.
