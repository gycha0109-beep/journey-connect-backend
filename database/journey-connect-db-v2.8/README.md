...
The backend Testcontainers bootstrap mirrors these reviewed bytes under the historical global execution labels `55`, `55a`, `56`, `57`, `58`, `59`, `59a`, and `60` so existing bootstrap ordering remains deterministic without extending the frozen v2.7 inventory.

The application login is not required to hold `jc_reliability` before writer activation. The role is an optional routed capability until a separate activation decision grants membership to the restricted backend login.
